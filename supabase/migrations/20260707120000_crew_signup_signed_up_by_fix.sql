-- Align crew_signups.signed_up_by with the values the app actually writes.
-- The RPC and every read path use 'self' / 'staff'. The old check allowed
-- 'student' / 'parent', so every signup failed with a check_violation and 500'd.
ALTER TABLE public.crew_signups
  DROP CONSTRAINT IF EXISTS crew_signups_signed_up_by_check;
ALTER TABLE public.crew_signups
  ADD CONSTRAINT crew_signups_signed_up_by_check
  CHECK (signed_up_by IN ('self', 'staff'));

-- Recreate the signup RPC with a check_violation guard so any future
-- data mismatch returns a clean error instead of a raw 500.
...
begin
  ...
  select s.id, s.capacity, r.name as role_name
  into v_slot
...
-- Direct execute stays locked to service_role (public flow goes through the Next.js API route).
