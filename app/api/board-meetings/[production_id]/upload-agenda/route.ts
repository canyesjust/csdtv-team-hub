...
import { callExtractAgendaEdge, parseExtractedAgenda } from '@/lib/board-meetings/meeting-api'
...
export async function POST(
  ...
  return withBoardMeetingProduction(params, async ({ service, teamUser, productionId }) => {
    ...
    try {
      ...
      if (livestream) {
        await service.from('productions').update({ livestream_url: livestream }).eq('id', productionId)
      }
