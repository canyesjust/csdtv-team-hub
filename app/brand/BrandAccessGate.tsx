...
// Shown by the /brand server layout when the visitor has not unlocked the site.
// On success the access cookie is set and we reload so the layout re-renders the
// real page.
export default function BrandAccessGate() {
  ...
  return (
    <div style={{ background: colors.bg, minHeight: '100vh', color: colors.text, fontFamily: 'system-ui, -apple-system, sans-serif', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div style={{ width: '100%', maxWidth: 480, background: colors.cardBg, border: `1px solid ${colors.line}`, borderRadius: 16, padding: '32px 28px', boxShadow: '0 10px 40px rgba(15,25,45,0.06)' }}>
        <h1 style={{ margin: 0, fontSize: 24, fontWeight: 800, lineHeight: 1.15 }}>Canyons School District Brand Library</h1>
        <p style={{ margin: '12px 0 0', fontSize: 14.5, lineHeight: 1.55, color: colors.muted }}>
          This site hosts official school and district logos, brand colors, and letterhead for Canyons School District. Access is restricted to approved partners and staff.
        </p>
        <p style={{ margin: '10px 0 0', fontSize: 14.5, lineHeight: 1.55, color: colors.muted }}>
          Need access? Please contact the Canyons School District Communications department to request the password.
        </p>
