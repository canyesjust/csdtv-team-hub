...
export default function SchoolBrandPage() {
  ...
  const mergeSelected = async () => {
    if (!reviewKey || !selected || !mergeTarget) return
    ...
    try {
      const res = await fetch('/api/brand/merge', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ code, key: reviewKey, fromCategory: selected.category, fromName: selected.name, toCategory, toName }),
      })
            ...
            <div style={{ padding: 18 }}>
              ...
              {reviewKey && (
                <>
                  ...
                  <p style={{ margin: '20px 0 6px', fontSize: 12, color: colors.muted, textTransform: 'uppercase', letterSpacing: '0.04em', fontWeight: 700 }}>Combine with another logo</p>
                  <p style={{ margin: '0 0 8px', fontSize: 12.5, color: colors.muted }}>
                  ...
                  {mergeOptions.length === 0 ? (
                    <p style={{ margin: 0, fontSize: 12.5, color: colors.muted }}>No other logos to combine with.</p>
                  ) : (
                    <>
                      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                        <select
                          ...
                          style={{ flex: '1 1 220px', height: 36, borderRadius: 8, border: `1px solid ${colors.line}`, background: colors.cardBg, color: colors.text, fontSize: 13, padding: '0 10px' }}
                        >
                        ...
                        <button
                          ...
                          style={{ padding: '8px 16px', borderRadius: 8, border: `1px solid ${colors.line}`, background: colors.cardBg, color: mergeTarget ? colors.info : colors.muted, fontSize: 13, fontWeight: 700, cursor: mergeBusy || !mergeTarget ? 'default' : 'pointer' }}
                        >
                          {mergeBusy ? 'Combining...' : 'Combine'}
                        </button>
                      </div>
                      {mergeMsg && <p style={{ margin: '8px 0 0', fontSize: 12.5, fontWeight: 600, color: colors.muted }}>{mergeMsg}</p>}
                    </>
                  )}
                </>
              )}
