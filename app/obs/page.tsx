    ...
    <div style={{ background: colors.bg, minHeight: '100vh', color: colors.text, fontFamily: 'system-ui, -apple-system, sans-serif', padding: '32px 20px' }}>
      <div style={{ maxWidth: 760, margin: '0 auto' }}>
        ...
        <section style={{ background: colors.cardBg, border: `1px solid ${colors.line}`, borderRadius: 14, padding: '20px 22px', marginTop: 18 }}>
          ...
          <p style={stepStyle}><strong>6. Set the password and scene.</strong> Open the <span style={codeStyle}>CSDtv Ad Controller</span> folder in your <strong>Documents</strong>, open <span style={codeStyle}>config.json</span>, and type the shared password between the quotes on the <span style={codeStyle}>&quot;password&quot;</span> line, the same one you used to open this page. Make sure <span style={codeStyle}>&quot;scene&quot;</span> matches your OBS scene name (it defaults to <span style={codeStyle}>Live</span>), or ads load but stay hidden. Save, then quit and reopen the app so it picks up the changes.</p>
          ...
          <p style={{ ...stepStyle, marginBottom: 0 }}><strong>8. Load the commercials.</strong> In the Ad Control dock, click <strong>Download missing</strong> and every commercial drops straight into the <span style={codeStyle}>ads</span> folder for you (inside <span style={codeStyle}>Documents/CSDtv Ad Controller</span>). No downloading and moving files by hand. Run it again anytime to pick up new ones. For the pre-show, put your starting-soon video in that folder&apos;s <span style={codeStyle}>preshow</span> folder and click <strong>Reload folder</strong>.</p>
        </section>
