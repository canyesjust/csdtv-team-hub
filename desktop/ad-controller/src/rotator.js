...
// Fallback readable name from a filename when the hub has no name for it.
...
export class Rotator extends EventEmitter {
  ...
  reload(quiet) {
    ...
    // Display names from the hub (filename -> renamed name), written by the sync.
    let names = {};
    try {
      names = JSON.parse(fs.readFileSync(path.join(path.dirname(this.cfg.adFolder), 'names.json'), 'utf8')) || {};
    } catch { names = {}; }

    const before = this.ads.map(a => a.file).join('|');
    try {
      this.ads = fs.readdirSync(this.cfg.adFolder)
        .map(f => {
          ...
          else if (imagesOn && IMAGE_EXTS.has(ext)) type = 'image';
          if (!type) return null;
          return { file: f, type, label: names[f] || prettyName(f) };
        })
        .filter(Boolean)
    ...
    // Only log when something actually changed, so a background refresh timer
    // doesn't spam the activity log.
    const changed = this.ads.map(a => a.file).join('|') !== before;
    if (!quiet || changed) {
      const vids = this.ads.filter(a => a.type === 'video').length;
      const imgs = this.ads.filter(a => a.type === 'image').length;
      this.emit('log', `Loaded ${this.ads.length} ad(s) (${vids} video, ${imgs} image)`);
    }
  ...
  status() {
    return {
      ...
      nowPlayingLabel: this.nowPlayingLabel || this.nowPlaying,
      nextInSec: this.nextAt ? Math.max(0, Math.round((this.nextAt - Date.now()) / 1000)) : null,
      ads: this.ads.map(a => ({ file: a.file, type: a.type, label: a.label || a.file })),
